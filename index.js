...
function isNamedColor(color) {
  if (window) {
    ...
    return color in colorName[color];
  }
}
...
function rgbaToHSL(color) {
  ...
  var hslAry = rgbToHsl(getLimitedNumber(rgbAry[0], 255, 0), getLimitedNumber(rgbAry[1], 255, 0), getLimitedNumber(rgbAry[2], 255, 0));
...
var Color = function () {
  function Color() {
    var cssColor = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { r: 0, g: 0, b: 0, h: 0, s: 0, l: 0, a: 0 };
    ...
    value: function toSTRING() {
      if (this.A === 0) {
        ...
        for (var key in colorName) {
          if (colorName[key][0] === this.R && colorName[key][1] === this.G && colorName[key][2] === this.B) {
            return key;
          }
        }
      }
